...
from schemas.game import GameStartResponse, MoveRequest, MoveResponse, UndoResponse, GameRecordResponse
...
async def start_ai_game(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    game = GomokuGame()
...
@router.post("/ai/move", response_model=ResponseModel[MoveResponse])
async def make_ai_move(
    ...
    game_record = db.query(GameRecord).filter(GameRecord.id == move_data.game_id).first()
    if game_record:
...
async def undo_ai_move(
    ...
    return ResponseModel(
        data=UndoResponse(success=True, message="撤销成功")
    )
...
def get_game_history(
    ...
    total = db.query(GameRecord).filter(
        (GameRecord.player1_id == current_user.id) | (GameRecord.player2_id == current_user.id)
    ).count()
    
    games = db.query(GameRecord).filter(
        (GameRecord.player1_id == current_user.id) | (GameRecord.player2_id == current_user.id)
    ).order_by(GameRecord.created_at.desc()).offset((page - 1) * page_size).limit(page_size).all()
