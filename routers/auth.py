...
def register(user_data: UserCreate, db: Session = Depends(get_db)):
    ...
    return ResponseModel(
        code=201, message="注册成功", data=UserResponse.model_validate(user)
    )
...
def login(user_data: UserLogin, request: Request, db: Session = Depends(get_db)):
    ...
    rate_limit_key = f"{user_data.username.lower()}:{client_host}"
    allowed, _, ttl = state_store.check_login_rate_limit(rate_limit_key)
    if not allowed:
        raise HTTPException(
    ...
    state_store.reset_login_rate_limit(rate_limit_key)
