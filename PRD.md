...
| 4 | **功能单一** — 无好友、无法观战 | 房间对战、6 位房间码、链接分享、实时围观 |
...
| 9 | 实时围观 | 大厅展示进行中房间；登录用户可用 `?watch=` 链接只读实时观看 |
...
| GET  | `/watch/{room_code}` | 是 | 进入进行中房间的只读观战会话 |
...
| `role` | S→C | `{ role: "observer" }`，观战者只读身份 |
...
观战者接收完整棋盘快照、落子、实际生效的悔棋和结束状态；不允许发送落子、悔棋或超时消息。对局结束后仅显示结果，不提供历史复盘。
